...
void Interrupt::OneTick()
{
    ...
    if (yieldOnReturn)
    {   // If the timer device handler asked for a
...
void Interrupt::Halt()
{
    ...
    // stats->Print(); Editamos para imprimir las estadisticas cuando ponemos ctrl+c
...
void Interrupt::SetStatus(MachineStatus st)
{
    status = st;
}
