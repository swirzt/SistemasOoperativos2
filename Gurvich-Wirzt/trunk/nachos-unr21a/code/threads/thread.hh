...
// Para que funcione la compilaciooon, Channel esta definido en channel.hh
class Channel;
...
public:
    /// Initialize a `Thread`.
    Thread(const char *debugName, bool join, int priority);
    ...
    /// Waits for a forked thread to finish
    int Join();

    /// Sets prio for a thread
    void SetPrio(int priority);

    /// Gets prio for a thread
    int GetPrio();
...
private:
    ...
    // Si joinable es False => canal = nullptr siempre
    bool joinable;
    Channel *canal = nullptr;
...
public:
    // Add a file descriptor to open files
