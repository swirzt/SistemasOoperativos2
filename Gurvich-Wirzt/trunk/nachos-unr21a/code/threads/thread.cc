...
Thread::Thread(const char *threadName, bool join, int priority)
{
    name = threadName;
    stackTop = nullptr;
    stack = nullptr;
    ...
    prio = priority;
    if (joinable)
        canal = new Channel("Comunicar con su padre");
#ifdef USER_PROGRAM
    space = nullptr;
...
Thread::~Thread()
{
    ...
    // if (joinable) Traía problemas, encotnrar otra forma de no perder espacio
    // {
    //     delete canal;
    // }
#ifdef USER_PROGRAM
...
void Thread::SetPrio(int priority)
{
    prio = priority;
}
...
int Thread::createFile(OpenFile *file)
{
